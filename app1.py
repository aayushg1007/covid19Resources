...
class Test(MDApp):
    ...
    def gen_link(self):
        i=0
        self.dialog = MDDialog(title="Link", text=tips, buttons=[
            MDFlatButton(
                text="CLOSE", 
                text_color=self.theme_cls.primary_color, 
                on_release= lambda x: self.dialog.dismiss())])
        #self.dialog = MDDialog(text=tips, buttons=MDFlatButton(text="CLOSE", text_color=self.theme_cls.primary_color, on_release=self.dialog.dismiss()))
        self.dialog.open()
